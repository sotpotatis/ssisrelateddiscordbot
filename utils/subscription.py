...
from utils.general import get_json, write_json, SUBSCRIPTIONS_DATA_FILEPATH, SUBSCRIPTIONS_SCHEMA_FILEPATH, BASE_TIMEZONE
...
def change_subscriber_status(category_name:str, subcategory_name:str, user:Member, add=True):
    ...
    if add and not subscribed or not add and subscribed:
        ...
        if add:
            subscription_data["subscriptions"][category_name][subcategory_name]["subscriptions"][str(user.id)] = {
                "last_notified_at": None
            }
        else:
            del subscription_data["subscriptions"][category_name][subcategory_name]["subscriptions"][str(user.id)]
        update_subscriptions(subscription_data)
        logger.info("User was added/removed to subscription.")
    else:
        logger.warning("User is already subscribed or unsubscribed! You should create a check for this and handle it somewhere else.")
...
def get_users_not_notified_after(timestamp, category_name, subcategory_name):
    ...
    logger.debug(f"Getting users not notified after {timestamp} in {category_name}/{subcategory_name}...")
    # Get the category
    subcategory_data = get_subscriptions()["subscriptions"][category_name][subcategory_name]["subscriptions"]
    subscribers_to_notify = []
    for user_id, subscription_data in subcategory_data.items():
        if subscription_data["last_notified_at"] != None:
            if datetime.datetime.fromisoformat(subscription_data["last_notified_at"]).astimezone(tz=pytz.timezone(BASE_TIMEZONE)) >= timestamp:
                continue
...
for category_name, category_data in subscriptions_schema.items():
    ...
    for subcategory in category_data["subcategories"]:
        if subcategory not in subscriptions["subscriptions"][category_name]:
            ...
            subscriptions["subscriptions"][category_name][subcategory] = {"subscriptions": {}}
            file_changed = True
    if file_changed:
