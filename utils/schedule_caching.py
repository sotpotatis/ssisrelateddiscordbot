...
from utils.general import write_json, get_json, get_active_classes, CACHED_SCHEDULE_DATA_FILEPATH, get_now, BASE_TIMEZONE
...
CACHED_SCHEDULE_TIMEOUT = 60 * 60 * 12 #Value in seconds - require caching at least every 12 hours (60 sec * 60 min * 12 hours)
MINIMUM_TIME_BETWEEN_CACHES = 60 * 15 #Value in seconds - allow caching max once every 15 minutes
...
async def cache_schedules():
    ...
    last_cached_at_parsed = datetime.datetime.fromisoformat(last_cached_at).astimezone(BASE_TIMEZONE) if last_cached_at != None else None
    if last_cached_at_parsed == None or (now-last_cached_at_parsed).total_seconds() >= MINIMUM_TIME_BETWEEN_CACHES:
        ...
        for class_to_retrieve in classes_to_retrieve:
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get("https://api.ssis.nu/cal", params={"room": class_to_retrieve},
                                           headers={"User-Agent": "Python/SSIS Discord Bot Schedule Parser"}) as request:
                        if request.status == 200:
                            ...
                            logger.critical(f"Request to SSIS API failed with status code {request.status}.")
                            raw_class_schedule = {}
                    await session.close()
                except Exception as e:
                    logger.critical(f"Something failed in the request to the SSIS API (error {e} occurred).", exc_info=True)
                    raw_class_schedule = {}
        ...
        logger.critical(f"Will not cache schedules - time since last cache is too little! {last_cached_at_parsed} was last cache time.")