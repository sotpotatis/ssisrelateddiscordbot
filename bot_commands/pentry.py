...
from utils.pentry import (
    get_pentryansvar_data,
    get_pentryansvar,
    write_pentryansvar_data,
)
...
class Pentry(Cog):
    ...
    async def pentryansvar_task_loop(self):
        ...
        if get_now().isoweekday() <= 7:
            ...
            cached_pentryansvar_data = (
                get_pentryansvar_data()
            )  # This is some fluid_data that we have saved about the previous message
            if pentryansvar_data == None:
                logger.warning(
                    "Pentryansvar fluid_data is not available. Check will be skipped."
                )
                return  # Stop the task
            pentryansvar_message_channel = self.bot.get_channel(
                cached_pentryansvar_data["pentry_information_channel_id"]
            )
            ...
            * Old week --> Check if cached fluid_data is different --> If yes, update the old message
            """
            last_information_message_sent_for_week = cached_pentryansvar_data[
                "information_message"
            ]["for_week"]
            ...
            if last_information_message_sent_for_week == current_week:
                ...
                if cached_pentryansvar_data["cached_data"] != pentryansvar_data:
                    logger.info(
                        "Data on server has changed! The message will be updated."
                    )
                    send_new_information_message = False
                else:
                    logger.info(
                        "The fluid_data has not been changed. The message will not be updated."
                    )
                    return  # Stop the task
            else:
                logger.info(
                    "Message for this week has not been sent. A new one will be generated."
                )
            ...
            final_message.set_footer(
                text=f"Data: Pentry API by 20alse | Meddelande uppdaterat {get_now().strftime('%Y-%m-%d %H:%M')}"
            )
            ...
            guild = self.bot.get_guild(
                cached_pentryansvar_data["find_roles_in_guild"]
            )  # Get the guild that we are using for roles
            logger.debug("Guild retrieved.")
            for pentry in pentryansvar_data:
                # Try to find the respective roles for the responsible class
                responsible_class = await class_name_to_role(
                    self.bot, guild, pentry["responsible_class"]
                )
                if responsible_class == None:
                    logger.info("Class role not available.")
                    responsible_class = pentry[
                        "responsible_class"
                    ].capitalize()  # Use the string provided by the API
                else:
                ...
                for person_name in pentry["responsible_persons"]:
                    ...
                    responsible_persons += " %s" % (
                        person_tag.mention if person_tag != None else person_name + ","
                    )
                responsible_persons = responsible_persons.strip(
                    ","
                ...
                if len(responsible_persons) > 0:
                    ...
                    logger.info(
                        "Information about responsible persons in the pentry not available."
                    )
                final_message.add_field(
                    name=f"➡ {pentry['pentry_name']}",
            ...
            if send_new_information_message:
                ...
                pentry_message = await pentryansvar_message_channel.send(
                    embed=final_message
                )
                ...
                previous_message = await pentryansvar_message_channel.fetch_message(
                    cached_pentryansvar_data["information_message"]["id"]
                )
                await previous_message.edit(embed=final_message)
                logger.info("Previous message edited.")
            cached_pentryansvar_data[
                "cached_data"
            ] = pentryansvar_data  # Save cached fluid_data
            cached_pentryansvar_data["message_last_updated_at"] = str(
                get_now()
            )  # Update update date
