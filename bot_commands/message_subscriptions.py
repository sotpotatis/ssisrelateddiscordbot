...
class SubscribedMessagesSubscription(Cog):
    ...
    async def subscribe_to_message(self,
                     interaction: Interaction,
                     category: str = SlashOption(name="kategori",
                                                 description="Meddelandekategori att prenumerera på"),
                     subcategory: str = SlashOption(name="meddelandetyp",
                                                    description="Meddelandetyp att prenumerera på")):
        ...
        if subscription.is_subscribed_to(interaction.user, category, subcategory):
            ...
            error_embed = generate_error_embed("Redan prenumererad", "Du har redan prenumererat på denna meddelandekanal.")
            await interaction.response.send_message(embed=error_embed, delete_after=60)
            return
        self.logger.info("Subscribing user...")
        subscription.change_subscriber_status(category, subcategory, interaction.user, True)
        ...
        final_embed.add_field(name="Viktigt!", value="""Se till att jag kan slidea in i dina DMs... 😉 
        Om du har blockerat mig eller har utökade sekretessinställningar på kommer du inte att få meddelanden från mig.""", inline=False)
        final_embed.set_footer(text="Psst! Detta meddelande raderas automatiskt efter 60 sekunder.")
    ...
    async def unsubscribe_to_message(self,
                                   interaction: Interaction,
                                   category: str = SlashOption(name="kategori",
                                                               description="Meddelandekategori att prenumerera på"),
                                   subcategory: str = SlashOption(name="meddelandetyp",
        ...
        if not subscription.is_subscribed_to(interaction.user, category, subcategory):
            ...
            error_embed = generate_error_embed("Inte prenumererad",
                                               "Aj då! Jag kan inte avprenumerera dig, för det verkar som att du inte är prenumererad!")
            await interaction.response.send_message(embed=error_embed, delete_after=60)
            return
        self.logger.info("Unsubscribing user...")
        subscription.change_subscriber_status(category, subcategory, interaction.user, False)
    ...
    async def autocomplete_category(self, interaction: Interaction, category:str):
        ...
        if not category: #Provide whole list of categories if none exist.
            ...
            await interaction.response.send_autocomplete(list(subscriptions["subscriptions"].keys()))
        else: #If a category ID has been set
            self.logger.debug("Autocompleting categories by closest values...")
            closest_category_ids = [available_category for available_category in subscriptions["subscriptions"].keys() if available_category.startswith(category)]
    ...
    async def autocomplete_subcategory(self, interaction:Interaction, subcategory:str, category:str):
        subscriptions = subscription.get_subscriptions()
        if not subcategory:
            ...
            if category:
                ...
                self.logger.debug("Autocompleting subcategory list by closest values...")
                closest_subcategory_ids = [available_subcategory for available_subcategory in subcategories if available_subcategory.startswith(subcategory)]
