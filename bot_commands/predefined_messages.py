...
class PredefinedMessages(Cog):
    ...
    async def autocomplete_predefined_messages(self, interaction: Interaction, message_id: str):
        ...
        if not message_id: #If a message ID has not been provided yet
            logger.debug("Message ID has not been autocompleted yet. Providing a whole list...")
            ...
            closest_predefined_message_ids = [predefined_message_id for predefined_message_id in predefined_message_ids if message_id.startswith(predefined_message_id)]
