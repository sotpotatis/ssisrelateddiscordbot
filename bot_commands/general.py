...
HEALTHCHECKS_PING_FREQ = (
    int(os.environ.get("HEALTHCHECKS_PING_FREQ"))
    if "HEALTHCHECKS_PING_FREQ" in os.environ
    else 2
)  # Default ping frequency is every 2 minutes
logger.info(
    f"Healthchecks settings: Ping URL: {HEALTHCHECKS_PING_URL}, ping frequency: {HEALTHCHECKS_PING_FREQ} (minutes)"
)
...
class General(Cog):
    def __init__(self, bot):
        ...
        if HEALTHCHECKS_PING_URL != None:
            logger.info(
                "A Healthchecks ping URL has been specified. Healthchecks will be enabled."
            )
            self.report_ping_to_healthchecks.start()  # Start the task for Healthchecks
        else:
            logger.warning(
                "A Healthchecks ping URL has not been specified. (You can ignore this message unless you intend to track the bot using Healthchecks)"
            )
    ...
    async def change_status(self):
        ...
        if random.randint(1, 2) == 1 or self.bot.activity is None:
            ...
            logger.info(
                f"Changing status to {new_status['type']}, {new_status['text']}."
            )
    ...
    async def eval(self, ctx, command: str):
        ...
        try:
            result = eval(command)
            final_embed = Embed(
                title="Eval har körts", description=f"Resultat: `{result}`"
            )
            logger.info("Eval har körts.")
        except Exception as e:
            logger.info(f"Eval misslyckades ({e})")
            final_embed = generate_error_embed(
                "Eval misslyckades", f"Ett fel inträffade: {e}."
            )
