...
class Clubs(commands.Cog):
    ...
    @nextcord.slash_command(description="Prenumerera på en klubb och få notiser när klubben har något att meddela.")
    async def subscribe_to_club(self,
        ...
        final_embed.add_field(name="Avprenumerera",
                              value=f"För att avprenumerera från meddelanden, använd kommandot `/unsubscribe_to_club {club_id}`.",
                              inline=False)
        final_embed.set_footer(text="Psst! Detta meddelande kommer att tas bort efter 60 sekunder.")
        message = await interaction.response.send_message(embed=final_embed, delete_after=60)
    ...
    async def unsubcribe_to_club(self,
                                 interaction: Interaction,
                                 club_id: str = SlashOption(name="klubb",
    ...
    @nextcord.slash_command(description="Detta kommando listar alla klubbar som du kan prenumerera på.")
    async def list_clubs(self,
                         interaction: Interaction):
        logger.info("Got a request to list clubs!")
        #Create a pretty list of clubs
        clubs = get_clubs_data()
        final_embed = Embed(title="Klubbar",
