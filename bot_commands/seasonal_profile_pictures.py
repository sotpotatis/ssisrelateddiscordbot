...
class SeasonalProfilePictures(commands.Cog):
    ...
    async def check_profile_picture_is_correct(self, *args, **kwargs):
        ...
        if not os.path.exists(SEASONAL_PROFILE_PICTURES_FILEPATH):
            self.logger.warning("No seasonal profile picture path is set! Seasonal profile pictures will be disabled.")
            return
        ...
        if currently_active_profile_picture is None:
            if "fallback" in seasonal_profile_pictures:
                ...
                self.logger.info("No currently active profile picture, but we have a fallback.")
            else:
            ...
            self.logger.info("Found currently active profile picture!")
        # Open image
        image_data = open(os.path.join(SEASONAL_PROFILE_PICTURES_DIRECTORY, currently_active_profile_picture["filename"]), "rb").read()
        await self.bot.wait_until_ready()
        await self.bot.user.edit(avatar=image_data)
        self.logger.info(f"Active image was ensured to {currently_active_profile_picture['filename']}.")
